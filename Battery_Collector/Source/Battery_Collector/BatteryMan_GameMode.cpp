...
void ABatteryMan_GameMode::SpawnPlayerRecharge() {
	auto RandX = FMath::RandRange(Spawn_X_Min, Spawn_X_Max);
	auto RandY = FMath::RandRange(Spawn_Y_Min, Spawn_Y_Max);

	auto SpawnPosition = FVector(RandX, RandY, Spawn_Z);
	auto SpawnRotation = FRotator(0.0f, 0.0f, 0.0f);
